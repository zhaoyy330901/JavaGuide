        ...
        implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
    ...
    /**
     *用户默认空实例的共享空数组
     *把它从EMPTY_ELEMENTDATA数组中国区分出来,以知道在添加第一个元素时容量需要扩容多少
     */
     ...
     * 保存ArrayList的数组
     */
    transient Object[] elementData; // 非私有的,transient修饰不被序列化
    ...
    public ArrayList(int initialCapacity) {
        if (initialCapacity > 0) { //当大于0时,new新的数组赋值给elementData
    ...
    /**
     判断size是否为0,若不为0,则根据elementData复制数组
     */
    ...
    private void grow(int minCapacity) {
        // oldCapacity 为旧容量, newCapacity为新容量
        int oldCapacity = elementData.length;
        //将oldCapacity右移一位, 其效果相当于oldCapacity/2,位运算效率高
        int newCapacity = oldCapacity + (oldCapacity >> 1);
        //检查新容量是否大于最小需要容量,若还是小于最小需要容量,那么就要把最小需要容量作为数组的新容量
    ...
    /**
     * 返回次列表中指定元素的首次出现的索引,如果此列表不包含此元素,则为-1
     */
